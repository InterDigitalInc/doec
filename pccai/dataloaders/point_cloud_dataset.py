...
from pccai.dataloaders.lidar_loader import LidarSimple, LidarSpherical, LidarOctree, LidarBlkOctArray, LidarOctArray, LidarOctVoxelContext
import torch
import numpy as np
...
def get_point_cloud_dataset(dataset_name):
    ...
    elif dataset_name.lower().find('blk_oct_array') >= 0:
        dataset_class = LidarBlkOctArray
    elif dataset_name.lower().find('oct_array') >= 0:
        dataset_class = LidarOctArray
    elif dataset_name.lower().find('oct_voxel_context') >= 0:
        dataset_class = LidarOctVoxelContext
    else:
        dataset_class = None
