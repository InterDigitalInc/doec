...
def train_one_minibatch(pccnet, points, points_raw, points_gt, optimizer, aux_optimizer, opt):
    avg_mb_loss = {}
    for mbi in range(len(points)): #mini batch index
        optimizer.zero_grad()
        if aux_optimizer is not None:
            aux_optimizer.zero_grad()

        # Forward and backward for the main loss
        output = pccnet(points[mbi])

        loss = output['loss']
        if torch.isnan(torch.mean(loss['bpp_loss'])):
        ...
        # Forward and backward for the auxiliary loos
        if aux_optimizer is not None:
            aux_loss = pccnet.module.aux_loss()
            aux_loss.backward()
            aux_optimizer.step()
        ...
        for k in loss:
            avg_mb_loss[k] = avg_mb_loss.get(k, 0) + loss[k].item()
...
def train_one_epoch(pccnet, dataloader, optimizer, aux_optimizer, writer, batch_total, opt):
    ...
    for batch_id, points in enumerate(dataloader):
        if isinstance(points, list):
            ...
            points_raw = []
            points_gt = []
        if points.dim() == 4 and points.shape[0] == 1: #we have to do batching on each minibatch
            points = points.squeeze().cuda()
            avg_mb_loss = train_one_minibatch(pccnet, points, points_raw, points_gt, optimizer, aux_optimizer, opt)
            for k in avg_mb_loss.keys(): avg_mb_loss[k] = avg_mb_loss[k] / (len(points) + 1) # the average mini batch loss

            # Log the results
            for k in avg_mb_loss:
                avg_loss[k] = avg_loss.get(k, 0) + avg_mb_loss[k]
            
            if batch_id % opt.print_freq == 0:
                message = '    batch count: %d/%d, iter: %d, ' % (batch_id, len_data, batch_id)
                for k in avg_mb_loss: 
                    message += '%s: %f, ' % (k, avg_mb_loss[k])
                    if writer is not None: writer.add_scalar('batch/' + k, avg_mb_loss[k], batch_total)
...
def validate_one_epoch(pccnet, dataloader, print_freq):
    ...
    for batch_id, points in enumerate(dataloader):
        if isinstance(points, list):
            ...
            points_raw = []
            points_gt = []
        if points.dim() == 4 and points.shape[0] == 1: #we have to do batching on each batch
            points = points.squeeze().cuda()
            avg_mb_loss = {}
            for mbi in range(len(points)): #mini batch index
                ...
                with torch.no_grad():
                    output = pccnet(points[mbi])
                    loss = output['loss']
                    for k, v in loss.items(): loss[k] = torch.mean(v.type(torch.FloatTensor))
                for k in loss:
                    avg_mb_loss[k] = avg_mb_loss.get(k, 0) + loss[k].item()
            for k in avg_mb_loss.keys(): avg_mb_loss[k] = avg_mb_loss[k] / (mbi + 1) # the average mini batch loss
            # Log the results
            if batch_id == 0:
                for k in avg_mb_loss: avg_loss[k] = avg_mb_loss[k]
            else:
                for k in avg_mb_loss: avg_loss[k] += avg_mb_loss[k]
