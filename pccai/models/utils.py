...
def get_SpConv3d_layer(dims, kernel_size, doLastRelu):
    layers = []
    for i in range(1, len(dims)):
        layers.append(ME.MinkowskiConvolution(in_channels=dims[i-1], out_channels=dims[i],
...
class SpConv3dLayers(nn.Sequential):
    ...
    def __init__(self, dims, kernel_size, doLastRelu=False):
        layers = get_SpConv3d_layer(dims, kernel_size, doLastRelu) # Note: may need to init the weights and biases here
        super(SpConv3dLayers, self).__init__(*layers)


def get_Conv3d_layer(dims, kernel_size, doLastRelu):
    layers = []
    for i in range(1, len(dims)):
        layers.append(nn.Conv3d(in_channels=dims[i-1], out_channels=dims[i],
            kernel_size=kernel_size, stride=1, padding=0, bias=True)) #padding=int((kernel_size-1)/2)
        if i==len(dims)-1 and not doLastRelu:
            continue
        layers.append(nn.ReLU(inplace=True))
    return layers

class Conv3dLayers(nn.Sequential):
    """3D convolutional layers.
    ...
    """
    def __init__(self, dims, kernel_size, doLastRelu=False):
        layers = get_Conv3d_layer(dims, kernel_size, doLastRelu) # Note: may need to init the weights and biases here
        super(Conv3dLayers, self).__init__(*layers)
